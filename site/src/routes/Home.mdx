...
1. **Drop-in UI components:** [`ShortcutsModal`], [`Omnibar`], [`SequenceModal`]
...
<div className="hint">
  Press <Kbd action="__hotkeys:modal" /> for shortcuts, <Kbd action="__hotkeys:omnibar" /> for command palette, or start a sequence (e.g. <code>G</code>…).
</div>
